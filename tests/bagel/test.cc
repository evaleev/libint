        ...
        for (unsigned int l3 = 0; l3 <= lmax; ++l3) {
          ...
          if (not do_timing_only) {
            ...
            // print out integrals to compare against Toru's Bagel
            {
              ...
              for(int i0=0, i0123=0; i0<nbf0; ++i0) {
                for(int i1=0; i1<nbf1; ++i1) {
                  for(int i2=0; i2<nbf2; ++i2) {
                    for(int i3=0; i3<nbf3; ++i3, ++i0123) {
                      ...
                      std::cout << i << " " << j << " " << k << " " << l << " "
                                << std::setprecision(15) << buffer[i0123] << std::endl;
                    }
                  }
                }
              }
...
//                    << " 0123=" << eribatch.swap0123() << std::endl;
          ...
          for(int k=0; k<nrepeats; ++k) {
            ...
            // print out integrals
            if (not do_timing_only) {
              const double* buffer = eribatch.data();
