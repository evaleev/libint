...
bagel::Resources* bagel::resources__;
bagel::MPI_Interface* bagel::mpi__;
#endif
...
#ifdef INCLUDE_BAGEL
    std::vector< std::shared_ptr<Shell> > bagel_basis;
    std::map<unsigned int, std::vector<unsigned int> > bagel_l_to_shells;
    std::vector< std::shared_ptr<Shell> > df_bagel_basis;
    std::map<unsigned int, std::vector<unsigned int> > df_bagel_l_to_shells;
    std::vector< std::shared_ptr<Shell> > bagelized_basis; // MPQC basis in Bagel format
    std::vector< std::shared_ptr<Shell> > bagelized_df_basis; // MPQC df_basis in Bagel format
    static const bool merge_shells_on_same_atom = true; // true corresponds to the Bagel represnetation of basis

    static std::tuple<std::vector< std::shared_ptr<Shell> >, std::map<unsigned int, std::vector<unsigned int> > >
           mpqc_to_bagel_basis(const sc::Ref<sc::GaussianBasisSet>& basis,
                               bool merge_shells);
#endif
};
TestBasis* testbs;
...
#ifdef INCLUDE_BAGEL
   bagel::mpi__ = new bagel::MPI_Interface(argc,argv);
#endif
...
# ifdef INCLUDE_BAGEL
   ...
   profile_bagel_4eri(deriv_order);
# endif
#endif
#ifdef INCLUDE_ERI3
   profile_3eri(deriv_order);
# ifdef INCLUDE_BAGEL
   profile_bagel_3eri(deriv_order);
# endif
#endif
#ifdef INCLUDE_ERI2
#endif
...
TestBasis::TestBasis()
{
  ...
  // create BAGEL bases
#ifdef INCLUDE_BAGEL
  std::tie(bagel_basis, bagel_l_to_shells) = mpqc_to_bagel_basis(basis, merge_shells_on_same_atom);
  std::tie(df_bagel_basis, df_bagel_l_to_shells) = mpqc_to_bagel_basis(df_basis, merge_shells_on_same_atom);
  std::tie(bagelized_basis, std::ignore) = mpqc_to_bagel_basis(basis, false);
  std::tie(bagelized_df_basis, std::ignore) = mpqc_to_bagel_basis(df_basis, false);
#endif
}

#ifdef INCLUDE_BAGEL
std::tuple<std::vector< std::shared_ptr<Shell> >, std::map<unsigned int, std::vector<unsigned int> > >
TestBasis::mpqc_to_bagel_basis(const sc::Ref<sc::GaussianBasisSet>& basis,
                               bool merge_shells) {
  ...
  if (merge_shells) {
    ...
    for(int c=0; c<ncenter; ++c) {
      ...
      for(unsigned int l=0; l<=lmax; ++l) {
        ...
        if (not ccoefs.empty()) {
          ...
          bagel_basis.push_back(std::shared_ptr<Shell>(new Shell(false, R,
                      l, exponents,
                      ccoefs, prim_range))
          );

#if 0
          std::cout << "bagel_shell " << bagel_basis.size() - 1
    ...
    for(int s=0; s<nshell; ++s) {
      ...
      bagel_basis[s] = std::shared_ptr<Shell>(new Shell(false, R,
              l, exponents,
              coefs, prim_range));
    }
  ...
  const unsigned int lmax = basis->max_angular_momentum();
  for(unsigned int l=0; l<=lmax; ++l) {
    bagel_l_to_shells[l] = std::vector<unsigned int>();
    for(int s=0; s<bagel_basis.size(); ++s) {
      if (bagel_basis[s]->angular_number() == l) {
        bagel_l_to_shells[l].push_back(s);
      }
    }

    std::cout<< "bagel_l_to_shells[" << l << "] = ";
    for(int k=0; k<bagel_l_to_shells[l].size(); ++k)
    std::cout << " " << bagel_l_to_shells[l][k] << " ";
    std::cout << std::endl;
  }

  return std::tie(bagel_basis, bagel_l_to_shells);
}
#endif
...
inline void prep_libint2(std::vector<LibintEval>& erievals,
                  const std::array<std::shared_ptr<const Shell>,4>& shells,
                  int norm_flag,
                  int deriv_order = 0) {
...
void profile_4eri(unsigned int deriv_order) {
  ...
  const auto& bbs = testbs->bagelized_basis;
...
void profile_3eri(unsigned int deriv_order) {
  ...
  const auto& bdfbs = testbs->bagelized_df_basis;
    ...
    for (unsigned int l1 = 0; l1 <= lmax; ++l1) {
      for (unsigned int l2 = 0; l2 <= lmax; ++l2) {
        for (unsigned int l3 = 0; l3 <= lmax; ++l3) {
          ...
          std::shared_ptr<Shell> s0(new Shell(false, R0,
                                              am[0], rsqset.exp[0][0],
...
void profile_bagel_4eri(unsigned int deriv_order) {
  ...
  const uint veclen = 1;
  const auto& bs = testbs->bagel_basis;
    ...
    for (unsigned int l1 = 0; l1 <= lmax; ++l1) {
      for (unsigned int l2 = 0; l2 <= lmax; ++l2) {
        for (unsigned int l3 = 0; l3 <= lmax; ++l3) {
          ...
          // loop over shell quartets of this type
          for (auto s0 = testbs->bagel_l_to_shells[l0].cbegin();
               s0 != testbs->bagel_l_to_shells[l0].cend();
               ++s0) {
            const auto& sh0 = bs[*s0];
            for (auto s1 = testbs->bagel_l_to_shells[l1].cbegin();
                 s1 != testbs->bagel_l_to_shells[l1].cend();
                 ++s1) {
              const auto& sh1 = bs[*s1];
              for (auto s2 = testbs->bagel_l_to_shells[l2].cbegin();
                   s2 != testbs->bagel_l_to_shells[l2].cend();
                   ++s2) {
                const auto& sh2 = bs[*s2];
                for (auto s3 = testbs->bagel_l_to_shells[l3].cbegin();
                     s3 != testbs->bagel_l_to_shells[l3].cend();
                     ++s3,
                     ++nshellsets) {
...
void profile_bagel_3eri(unsigned int deriv_order) {
  ...
  const uint veclen = 1;
  auto bs = testbs->bagel_basis;
  auto dfbs = testbs->df_bagel_basis;

  unsigned int lmax = max(testbs->basis->max_angular_momentum(), testbs->df_basis->max_angular_momentum());

  for (unsigned int l0 = 0; l0 <= lmax; ++l0) {
    if (testbs->df_bagel_l_to_shells[l0].empty()) continue;
    for (unsigned int l1 = 0; l1 <= lmax; ++l1) {
      if (testbs->bagel_l_to_shells[l1].empty()) continue;
      for (unsigned int l2 = 0; l2 <= lmax; ++l2) {
        if (testbs->bagel_l_to_shells[l2].empty()) continue;
          ...
          // loop over shell quartets of this type
          for (auto s0 = testbs->df_bagel_l_to_shells[l0].cbegin();
               s0 != testbs->df_bagel_l_to_shells[l0].cend();
               ++s0) {
