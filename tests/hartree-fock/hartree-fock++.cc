...
std::vector<Matrix> compute_2body_fock_deriv(
    const BasisSet& obs, const std::vector<Atom>& atoms, const Matrix& D,
    double precision = std::numeric_limits<
        double>::epsilon(),           // discard contributions smaller than this
    const Matrix& Schwarz = Matrix()  // K_ij = sqrt(||(ij|ij)||_\infty); if
                                      // empty, do not Schwarz screen
...
// prepare for density fitting
    ...
    for (int k = 0; k != 10; ++k)
      oct[k] = -2 * D.cwiseProduct(Mu[k + 10])
                        .sum();  // 2 = alpha + beta, -1 = electron charge
                                 //    std::cout << "** eoctupole = ";
    ...
    auto SMu = compute_1body_ints<Operator::sphemultipole>(obs);
    {
      ...
      std::cout
          << "C^3_0 - (2zzz - 3xxz - 3yyz)/12:\n"
          << ((2. * Mu[19] - 3. * Mu[12] - 3. * Mu[17]) / 12 - SMu[12]).norm()
          << std::endl;
#endif
    }
      ...
      for (auto s2 : obs_shellpair_list[s1]) {
        ...
        switch (deriv_order) {
          ...
          case 2: {
            ...
            auto upper_triangle_index = [](long n2, long i, long j) {
