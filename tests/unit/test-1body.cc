    ...
    if (lmax >= 2) {
      ...
      engine.compute(obs[0], obs[0]);
      {
        ...
        std::vector<double> shellset_ref_gaussian = {
            -1.343732979153083e+01, 0.000000000000000e+00,
            -6.796143730068988e-02, -1.478824785355970e-02,
            0.000000000000000e+00,  0.000000000000000e+00,
            -1.284475452992947e+01, 0.000000000000000e+00,
            0.000000000000000e+00,  -5.775996163160049e-02,
            -6.796143730068988e-02, 0.000000000000000e+00,
            -1.301230978657952e+01, 1.139389632827834e-01,
            0.000000000000000e+00,  -1.478824785355970e-02,
            0.000000000000000e+00,  1.139389632827834e-01,
            -1.241040347301479e+01, 0.000000000000000e+00,
            0.000000000000000e+00,  -5.775996163160049e-02,
            0.000000000000000e+00,  0.000000000000000e+00,
        ...
        for (int i = 0; i != 25; ++i) {
          if (libint2::solid_harmonics_ordering() ==
            ...
            REQUIRE(engine.results()[0][i] / scale ==
                    Approx(shellset_ref_standard[i]));
          } else {
            if (i == 0) printf("Checking 1-body SHO=Gaussian\n");
            REQUIRE(engine.results()[0][i] / scale ==
                    Approx(shellset_ref_gaussian[i]));
          }
        }
      }
      ...
      engine.compute(obs[0], obs[1]);
      {
        ...
        std::vector<double> shellset_ref_gaussian = {
            2.128355904665372e+00,  1.074590109905394e+00,
            1.074249956874296e+00,  -3.485163651594458e-03,
            -2.171951144148577e+00, 9.566621490332916e-01,
            -3.760919234260182e-01, -8.344173649626901e-01,
            1.660514988916377e+00,  -1.160865205880651e+00,
            9.572841308198474e-01,  -8.291498398421207e-01,
            -3.770080831197964e-01, -1.663667687168316e+00,
            -1.159266418436018e+00, -2.105750177166632e-03,
            1.380654897976564e+00,  -1.385603731947886e+00,
            2.115041199099945e+00,  -1.120272634615116e-03,
            -1.559058302243514e+00, -9.290824121864600e-01,
            -9.303619356400431e-01, -5.835786921473129e-04,
      ...
      Shell{{2.0, 5.0}, {{l1, true, {1.0, 0.2}}}, {{1.0, 1.0, 1.0}}}};
  {
    ...
    if (lmax >= 2) {
      ...
      engine.compute(obs[0], obs[1]);
      {
        ...
        if (libint2::solid_harmonics_ordering() ==
            libint2::SHGShellOrdering_Standard) {
          REQUIRE(engine.results()[0][i0i1_s] == Approx(shellset_ref[i0i1_s]));
        } else {
          ...
          REQUIRE(engine.results()[0][i0i1_g] == Approx(shellset_ref[i0i1_s]));
        }
        END_FOR_SOLIDHARM
        END_FOR_SOLIDHARM
      }
