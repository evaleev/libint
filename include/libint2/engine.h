...
#define BOOST_PP_NBODY_DERIV_ORDER_LIST \
 ...
 private:
  ...
  inline void compute_primdata(Libint_t& primdata, const Shell& s1,
  ...
  /// any(params)
  /// else will set return any initialized with default value for
  /// operator_traits<type>::oper_params_type
...
inline const std::vector<Engine::compute2_ptr_type>& Engine::compute2_ptrs()
    const {
...
template <typename Params>
inline any Engine::enforce_params_type(Operator oper, const Params& params,
...
inline void Engine::compute_primdata(Libint_t& primdata, const Shell& s1,
                                     const Shell& s2, size_t p1, size_t p2,
                                     size_t oset) {
  const auto& A = s1.O;
  const auto& B = s2.O;
      ...
      for (auto s = 0; s != ntargets; ++s) {
        ...
        if (permute) {
          ...
          if (deriv_order == 1) {  // generalize this mess
            if (swap_braket) s_target = (s + 6) % 12;
            if (s_target < 6) {
              if (swap_bra) {
