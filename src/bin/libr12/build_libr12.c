...
int main()
{
  ...
  /*----------------------------------------
   ...
   ----------------------------------------*/
  new_am = LIBR12_MAX_AM;
  if (new_am <= 0)
    punt("  MAX_AM must be positive.");
  if (new_am > LIBINT_MAX_AM - DERIV_LVL)
  ...
  Params.new_am = new_am*2;
  Params.old_am = 0;
  Params.opt_am = opt_am*2;
