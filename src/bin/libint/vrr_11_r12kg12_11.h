                                                                ...
                                                                vector<F>* bra_ref, vector<F>* ket_ref)
    {
      ...
      if (a_minus_2_exists) {
        ...
        if (is_simple()) {
          ...
          SafePtr<ExprType> expr01_ptr(new ExprType(ExprType::OperatorTypes::Plus,expr1_ptr,expr_));
          expr_ = expr01_ptr;
        }
      }
      ...
      if (c_minus_1_exists) {
        ...
        if (is_simple()) {
          ...
          SafePtr<ExprType> exprsum_ptr(new ExprType(ExprType::OperatorTypes::Plus,expr2_ptr,expr_));
          expr_ = exprsum_ptr;
        }
      }
	...
	throw std::logic_error("VRR_11_R12kG12_11<I,F,K,part,where>::children_and_expr_Kge0() -- AM on centers b and d must be zero, general RR is not yet implemented");
      ...
      if (EqualZero<K>::result == false) {
        ...
        if (is_simple()) {
          ...
          SafePtr<ExprType> exprsum_ptr(new ExprType(ExprType::OperatorTypes::Plus,expr_intmd4,expr_));
          expr_ = exprsum_ptr;
        }
      }
    }
