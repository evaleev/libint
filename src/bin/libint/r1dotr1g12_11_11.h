...
#ifndef _libint2_src_bin_libint_r1dotr1g121111_h_
#define _libint2_src_bin_libint_r1dotr1g121111_h_
...
namespace libint2 {
  ...
  /**
     R1dotR1G12_11_11 --
     integral over R1dotR1_G12 operator with one bfs for each particle in bra and ket.
  */
  template <class BFS> class R1dotR1G12_11_11 :
    ...
    public:
      ...
      typedef R1dotR1G12_11_11<BFS> this_type;

      /// R1dotR1G12_11_11 is a set of these subobjects
      typedef R1dotR1G12_11_11<typename BFS::iter_type> iter_type;
      ...
      typedef SingletonStack<R1dotR1G12_11_11,key_type> SingletonManagerType;

      /* This "constructor" takes basis function sets, in Mulliken ordering.
         Returns a pointer to a unique instance, a la Singleton
      */
      static const std::shared_ptr<R1dotR1G12_11_11> Instance(const BFS& bra0, const BFS& ket0, const BFS& bra1, const BFS& ket1);
      /// Returns a pointer to a unique instance, a la Singleton
      static const std::shared_ptr<R1dotR1G12_11_11> Instance(const BraType& bra, const KetType& ket, const AuxIndexType& aux);
      ~R1dotR1G12_11_11();
    ...
    private:
      ...
      R1dotR1G12_11_11(const BraType& bra, const KetType& ket, const AuxIndexType& aux);
  ...
  template <class BFS>
    typename R1dotR1G12_11_11<BFS>::SingletonManagerType
    R1dotR1G12_11_11<BFS>::singl_manager_(&R1dotR1G12_11_11<BFS>::key);
#else
#error "USE_INT_KEY_TO_HASH must be set"
#endif

  template <class BFS>
    R1dotR1G12_11_11<BFS>::R1dotR1G12_11_11(const BraType& bra, const KetType& ket,  const AuxIndexType& aux) :
    parent_type(R1dotR1_G12(),bra, ket, aux)
    {
      if (bra.num_members(0) != 1)
        throw std::runtime_error("R1dotR1G12_11_11<BFS>::R1dotR1G12_11_11(bra,ket) -- number of BFSs in bra for particle 0 must be 1");
      if (bra.num_members(1) != 1)
        throw std::runtime_error("R1dotR1G12_11_11<BFS>::R1dotR1G12_11_11(bra,ket) -- number of BFSs in bra for particle 1 must be 1");
      if (ket.num_members(0) != 1)
        throw std::runtime_error("R1dotR1G12_11_11<BFS>::R1dotR1G12_11_11(bra,ket) -- number of BFSs in ket for particle 0 must be 1");
      if (ket.num_members(1) != 1)
        throw std::runtime_error("R1dotR1G12_11_11<BFS>::R1dotR1G12_11_11(bra,ket) -- number of BFSs in ket for particle 1 must be 1");
#if DEBUG
      std::cout << "R1dotR1G12_11_11: constructed " << this->label() << std::endl;
#endif
    }

  template <class BFS>
    R1dotR1G12_11_11<BFS>::~R1dotR1G12_11_11()
    {
#if DEBUG
      std::cout << "R1dotR1G12_11_11: destructed " << this->label() << std::endl;
#endif
    }

  template <class BFS>
    const std::shared_ptr< R1dotR1G12_11_11<BFS> >
    R1dotR1G12_11_11<BFS>::Instance(const BraType& bra, const KetType& ket, const AuxIndexType& aux)
    {
      ...
      if (!val.second) {
std::shared_ptr<R1dotR1G12_11_11> this_int(new R1dotR1G12_11_11<BFS>(bra,ket,aux));
...
return val.second;
  ...
  template <class BFS>
    bool
    R1dotR1G12_11_11<BFS>::operator==(const R1dotR1G12_11_11<BFS>& a) const
    {
  ...
  template <class BFS>
    const std::string&
    R1dotR1G12_11_11<BFS>::label() const
    {
      if (label_.empty()) {
ostringstream os;
...
label_ = os.str();
  ...
  template <class BFS>
    bool
    R1dotR1G12_11_11<BFS>::this_precomputed() const
    {
      return false;
    }
